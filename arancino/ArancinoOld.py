    ...
    def run(self):
        ...
        while not self.__stop:
            if not self.__pause:
                ...
                for id, p_disc in self.__ports_discovered.items():
                    ...
                    if id in self.__ports_connected:
                        ...
                        if not self.__synchronizer.portExists(port=p_disc):
                            p_disc.setCreationDate(datetime.now())
                            self.__synchronizer.writePortConfig(p_disc)
                            self.__synchronizer.writePortBase(p_disc)
                        else:
                            # TODO read creation date
                            p_disc = self.__synchronizer.readPortConfig(p_disc)
                            p_disc = self.__synchronizer.readPortChanges(p_disc)


                        self.__synchronizer.writePortChanges(p_disc)
                        self.__synchronizer.writePortLink(p_disc)
                        self.__synchronizer.writePortStatus(p_disc)
                        self.__synchronizer.writePortInfo(p_disc)

                        if p_disc.isEnabled():
                            p_disc.setDisconnectionHandler(self.__disconnectedPortHandler)
                            p_disc.setReceivedCommandHandler(self.__commandReceived)
                            p_disc.connect()

                            # move Arancino Port to the self.__ports_connected
                            self.__ports_connected[id] = p_disc
